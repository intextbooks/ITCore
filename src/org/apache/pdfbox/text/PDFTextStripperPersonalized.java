...
import intextbooks.content.extraction.buildingBlocks.format.Paragraph;
...
import intextbooks.content.extraction.format.PdfTextExtractor;
 ...
 * 
 * Modified by @author alpiz001 for the ITCore project. Visit: https://intextbooks.science.uu.nl/
 * Changes are documented as comments
 */
public class PDFTextStripperPersonalized extends LegacyPDFStreamEngine
{
     ...
     */
    public PDFTextStripperPersonalized() throws IOException
    {
    ...
    /**
     * This method returns the document as ResourceUnits, the inner structure for the ITCore project.
     * 
     * @author alpiz001
     * @param doc
     * @return
     */
    public Vector<ResourceUnit> getDocAsResourceUnits(PDDocument doc){
    	StringWriter outputStream = new StringWriter();
        try {
			writeText(doc, outputStream);
		} catch (IOException e) {
			SystemLogger.getInstance().log("ERROR parsing the textbook: " + e.getMessage());
			e.printStackTrace();
		}
        this.completeText = outputStream.toString();
        return this.units;
    }
    ...
    protected void processPages(PDPageTree pages) throws IOException
    {
        ...
        {
        	SystemLogger.getInstance().log("Processing page " + currentPageNo);
            currentPageNo++;
            if (page.hasContents() && page.getContents() != null && page.getContents().available() != 0)
            {
                processPage(page);
            } else {
            	this.units.add(null);
    ...
    {  	
                ...
                && (endBookmarkPageNumber == -1 || currentPageNo <= endBookmarkPageNumber))
        {
            ...
            //IAC Start page
            this.currentResourceUnit = new ResourceUnit();
    		currentResourceUnit.setPageIndex(currentPageNo - 1);
    		units.add(currentResourceUnit);
            ...
            int rotation2 = page.getRotation();
            if (rotation2 != 0)
            {
     ...
     * enabled.
     * 
     * Modified by @author alpiz001 to use bounding boxes and improved sorting 
     *
    ...
    protected void writePage() throws IOException
    {
        ...
        for (List<TextPosition> textList : charactersByArticle)
        {
        	...
        	for(TextPosition text: textList) {
    			...
    			try {
    				rect = new Rectangle2D.Float(0, 0, text.getFont().getWidth(text.getCharacterCodes()[0]) / 1000, 1);
    			} catch (IOException e) {
    				e.printStackTrace();
    			}
            ...
            while (textIter.hasNext())
            {
                ...
                //get positions according to BBs
                BoundingBox currentBB = this.boundingBoxByTextPosition.get(position);
                if(currentBB == null) {
                	continue;
                }
                positionX = currentBB.getLowerLeftX();
                positionY = currentBB.getUpperRightY();
                positionWidth = currentBB.getWidth();
                positionHeight = currentBB.getHeight();
...
//                      System.out.println("boundingBoxesAreDisjoint(currentBB, lastBB) " + (boundingBoxesAreDisjoint(currentBB, lastBB))); 
//                      System.out.println("\tXS: " + currentBB.getLowerLeftX());
//                      System.out.println("\tXEnd: " + currentBB.getUpperRightX());
//                      System.out.println("\tY TOP: " + currentBB.getLowerLeftY());
//                      System.out.println("\tY BOT: " + currentBB.getUpperRightY());
//                      System.out.println("Last: " + lastPosition.getTextPosition().getUnicode());
//                      System.out.println("\tXS: " + lastBB.getLowerLeftX());
//                      System.out.println("\tXEnd: " + lastBB.getUpperRightX());
...
//                    	System.out.println("--- new line  END" );
                ...
                // IAC use the right limit of the bounding box as the end of last text x
                endOfLastTextX = currentBB.getUpperRightX();
    ...
    private boolean boundingBoxesAreDisjoint(BoundingBox bb1, BoundingBox bb2) {
    	if( (bb1.contains(bb2.getLowerLeftX(), bb2.getLowerLeftY())) ||
    			(bb1.contains(bb2.getUpperRightX(), bb2.getLowerLeftY())) ||
    			(bb1.contains(bb2.getUpperRightX(), bb2.getUpperRightY())) ||
    			(bb1.contains(bb2.getLowerLeftX(),  bb2.getUpperRightY()))) {
    		return false;
    	} else {
    		return true;
    	}
    ...
    private void sortUsingBoundingBoxes(List<TextPosition> textList) {
    	...
    	for(TextPosition text: textList) {
    		BoundingBox bB = this.boundingBoxByTextPosition.get(text);
    		Integer key = (int) bB.getUpperRightY(); //row or line level
			List<TextPosition> tmpList = lines.get(key);
			if(tmpList == null) {
...
//    	//System.exit(0);
    	...
    	for(int i = 0; i < arrayOfLines.length -1; i++) {
    		...
    		line:
    		for(TextPosition tP0: line0) {
    			BoundingBox bB0 = this.boundingBoxByTextPosition.get(tP0);
    			for(TextPosition tP1: line1) {
    				BoundingBox bB1 = this.boundingBoxByTextPosition.get(tP1);
    				if(this.overlap(bB0, bB1)) {
    					overlappingLines.add(i);
    					overlappingLines.add(i+1);
//    					/*TESTING*/
//    					System.out.println ("OV(" + i + "): " + getListAsString(line0) + " to " +  getListAsString(line1) );
//    					System.out.println ("\tC: " + tP0.getUnicode() + " YTop: " + bB0.getLowerLeftY() + " YDown: " + bB0.getUpperRightY());
//    					System.out.println ("\tC: " + tP1.getUnicode() + " YTop: " + bB1.getLowerLeftY() + " YDown: " + bB1.getUpperRightY());
//    					/*TESTING*/
    					break line;
    				}
    			}
...
//    	/*TESTING*/
    	...
    	//Every line is checked against adjacent lines (before and after) to find the best line to be merged with 
		Set<Integer>  processedLines = new HashSet<Integer>();
		int currentLineIndex = nextLineToProcess(overlappingLines, processedLines, arrayOfLines);
		int[] merge = {-1,-1};
		while(currentLineIndex != -1) {
			
//			/*TESTING*/
//			System.out.println("**************Processing: " + currentLineIndex);
//			for(Entry<Integer, List<TextPosition>> e : lines.entrySet() ) {
//	    		Collections.sort(e.getValue(), new TextPositionXComparator());
//	    		System.out.println("$ " + getListAsString(e.getValue()));
//	    	} 
//			/*TESTING*/
			...
			List<TextPosition> currentLine = arrayOfLines[currentLineIndex].getValue();
			Iterator<TextPosition> currentLineIterator = currentLine.iterator();
			while(currentLineIterator.hasNext()){
				TextPosition currentElement = currentLineIterator.next();
				if(mergedTextPositions.contains(currentElement)) {
					//System.out.println("skipping Element: " + currentElement.getUnicode());
					continue;
				}
				...
				TextPosition bestClosestElementPreviousLine = null;
				
				Integer previousLineIndex = currentLineIndex - 1;
    			while(overlappingLines.contains(previousLineIndex)) {
    				List<TextPosition> nextPreviousLine = arrayOfLines[previousLineIndex].getValue();
					TextPosition closestElementPreviousLine = getClosestXElement(currentElement, nextPreviousLine);
					if(closestElementPreviousLine != null && !mergedTextPositions.contains(closestElementPreviousLine)) {
						BoundingBox bbCurrentPreviousElement = this.boundingBoxByTextPosition.get(closestElementPreviousLine);
    					float sharedAreaToPreviousLine = getSharedY(bbCurrentElement, bbCurrentPreviousElement);
    					if(sharedAreaToPreviousLine > bestSharedAreaToPreviousLine) {
    						bestSharedAreaToPreviousLine = sharedAreaToPreviousLine;
    						bestClosestPreviousLine = previousLineIndex;
    						bestClosestElementPreviousLine = closestElementPreviousLine;
    					}
					}
    				previousLineIndex--;
    			}
...
//    			/*TESTING*/
//    			System.out.println("bestSharedAreaToPreviousLine: " + bestSharedAreaToPreviousLine);
//    			System.out.println("bestClosestPreviousLine: " + bestClosestPreviousLine);
//    			System.out.println("bestClosestPreviousElement: " + (bestClosestPreviousLine != 0 ? bestClosestElementPreviousLine.getUnicode() : "null"));
//    			/*TESTING*/
    			
    			float bestSharedAreaToFollowingLine = 0;
				int bestClosestFollowingLine = 0;
				TextPosition bestClosestElementFollowingLine = null;
				
    			Integer followingLineIndex = currentLineIndex + 1;
    			while(overlappingLines.contains(followingLineIndex)) {
    				List<TextPosition> nextFollowingLine = arrayOfLines[followingLineIndex].getValue();
					TextPosition closestElementFollowingLine = getClosestXElement(currentElement, nextFollowingLine);
					if(closestElementFollowingLine != null && !mergedTextPositions.contains(closestElementFollowingLine)) {
						BoundingBox bbCurrentFollowingElement = this.boundingBoxByTextPosition.get(closestElementFollowingLine);
    					float sharedAreaToFollowingLine = getSharedY(bbCurrentElement, bbCurrentFollowingElement);
    					if(sharedAreaToFollowingLine > bestSharedAreaToFollowingLine) {
    						bestSharedAreaToFollowingLine = sharedAreaToFollowingLine;
    						bestClosestFollowingLine = followingLineIndex;
    						bestClosestElementFollowingLine = closestElementFollowingLine;
    					}
					}
    				followingLineIndex++;
    			}
...
//    			/*TESTING*/
//    			System.out.println("bestSharedAreaToFollowingLine: " + bestSharedAreaToFollowingLine);
//    			System.out.println("bestClosestFollowingLine: " + bestClosestFollowingLine);
...
//					System.out.println("bestSharedAreaToPreviousLine: " +  bestSharedAreaToPreviousLine);
//					System.out.println("bestSharedAreaToPreviousLine > bestSharedAreaToFollowingLine");
//					/*TESTING*/
    				merge[0] = bestClosestPreviousLine;
					merge[1] = currentLineIndex;
    				break;
    			} else if(bestSharedAreaToFollowingLine > bestSharedAreaToPreviousLine) {
//					/*TESTING*/
//					System.out.println("bestSharedAreaToFollowingLine > bestSharedAreaToPreviousLine");
//					/*TESTING*/
    				merge[0] = bestClosestFollowingLine;
					merge[1] = currentLineIndex;
					break;
    			}
			}
			
			//merge the lines if necessary
			if(merge[0] != -1) {
				arrayOfLines[merge[0]].getValue().addAll(arrayOfLines[merge[1]].getValue());
				mergedTextPositions.addAll(arrayOfLines[merge[1]].getValue());
				arrayOfLines[merge[1]].getValue().clear();
			}
			
			//next line to process
			currentLineIndex = nextLineToProcess(overlappingLines, processedLines, arrayOfLines);
		}
		
		//cleaning overlapping spaces: a space occupies the same space area (or less) as the previous  character
		List<TextPosition> toRemove = new ArrayList<TextPosition>();
		for(Entry<Integer, List<TextPosition>> e : lines.entrySet() ) {
    		Collections.sort(e.getValue(), new TextPositionXComparator());
    		TextPosition lastTP = null;
    		Iterator<TextPosition> iterator = e.getValue().iterator();
    		while(iterator.hasNext()) {
    			TextPosition c = iterator.next();
    			//white character before
    			if(StringUtils.isWhitespace(c.getUnicode()) && lastTP != null) {
    				BoundingBox bbCurrentElement = this.boundingBoxByTextPosition.get(c);
    				BoundingBox bbLastElement = this.boundingBoxByTextPosition.get(lastTP);
    				if(bbCurrentElement.getLowerLeftX() >= bbLastElement.getLowerLeftX() && bbCurrentElement.getUpperRightX() <= bbLastElement.getUpperRightX()) {
    					iterator.remove();
    				}
    			}
    			//white character after
    			if(lastTP != null && StringUtils.isWhitespace(lastTP.getUnicode()) && !StringUtils.isWhitespace(c.getUnicode())) {
    				BoundingBox bbCurrentElement = this.boundingBoxByTextPosition.get(c);
    				BoundingBox bbLastElement = this.boundingBoxByTextPosition.get(lastTP);
    				if(bbLastElement.getLowerLeftX() >= bbCurrentElement.getLowerLeftX() && bbLastElement.getUpperRightX() <= bbCurrentElement.getUpperRightX()) {
    					toRemove.add(lastTP);
    				}
    			}
    			lastTP = c;
...
//    		Collections.sort(e.getValue(), new TextPositionXComparator());
//    		System.out.println("$ " + getListAsString(e.getValue()));
//    		for(TextPosition c: e.getValue()) {
//    			System.out.println("\t" + c.getUnicode() + " C: " + ((int) c.getUnicode().charAt(0)) + " SX: " + c.getX() + " EX: " + c.getEndX());
//    		}
//    	} 
//		System.exit(0);
//		/*TESTING*/
     ...
     * @return
     */
    private int nextLineToProcess(Set<Integer> overlappingLines, Set<Integer> processedLines, Entry<Integer, List<TextPosition>>[] arrayOfLines) {
    	int nextLine = -1;
    	Iterator<Integer> overlappingLinesIterator = overlappingLines.iterator();
    	int cantOfChars = 9999;
    	while(overlappingLinesIterator.hasNext()) {
    		int currentLineIndex = overlappingLinesIterator.next();
    		if(!processedLines.contains(currentLineIndex) && arrayOfLines[currentLineIndex].getValue().size() > 0 && arrayOfLines[currentLineIndex].getValue().size() < cantOfChars) {
    			cantOfChars = arrayOfLines[currentLineIndex].getValue().size();
     ...
     * @return
     */
    private String getListAsString(List<TextPosition> textList) {
    	//Collections.sort(textList, new TextPositionXComparator());
    	String l = "[";
     ...
     * @return
     */
    private boolean isFullOverlap(BoundingBox bb1, BoundingBox bb2)
    {
    	if(bb1.contains(bb2.getLowerLeftX(), bb2.getUpperRightY()) ||
    			bb1.contains(bb2.getLowerLeftX(), bb2.getLowerLeftY()) || 
    			bb1.contains(bb2.getUpperRightX(), bb2.getUpperRightY()) ||
    			bb1.contains(bb2.getUpperRightX(), bb2.getLowerLeftY()) ) {
					 return true;
				 }
     ...
     * @return
     */
    private boolean overlap(BoundingBox bb1, BoundingBox bb2)
    {
      float pos1YTop = bb1.getLowerLeftY();
      float pos1YBottom = bb1.getUpperRightY();
      float pos2YTop = bb2.getLowerLeftY();
      float pos2YBottom =  bb2.getUpperRightY();
    
        if((pos2YBottom >= pos1YTop && pos2YBottom <= pos1YBottom) || (pos1YBottom >= pos2YTop && pos1YBottom <= pos2YBottom)){
          return true;
        } else {
          return false;
        }
     ...
     * @return
     */
    private float getSharedY(BoundingBox bb1, BoundingBox bb2){
    	float pos1YTop = bb1.getLowerLeftY();
        float pos1YBottom = bb1.getUpperRightY();
        float pos2YTop = bb2.getLowerLeftY();
        float pos2YBottom =  bb2.getUpperRightY();
        ...
        if(pos2YBottom >= pos1YTop && pos2YBottom <= pos1YBottom) {
        	return pos2YBottom - pos1YTop;
        } else if (pos1YBottom >= pos2YTop && pos1YBottom <= pos2YBottom) {
        	return pos1YBottom - pos2YTop;
        } else {
        	return 0;
        }
    }
    ...
    /**
     * Find in the line the other elements that are close to the target element (either at the left or write form the target
     * by comparing the distance which should be less than the width of space), or that are fully overlapping (superposition).
     * From the close elements, it chooses the one with the greatest shared Y area. Elements also need to have
     * different font size, since this is oriented to find subscript and superscript characters.
     * 
     * @author alpiz001
	 *
     * @param element
     ...
     * @return
     */
    private TextPosition getClosestXElement(TextPosition element, List<TextPosition> list) {
    	BoundingBox bbElement = this.boundingBoxByTextPosition.get(element);
    	float elementXLeft = bbElement.getLowerLeftX();
    	float elementXRight = bbElement.getUpperRightX();
    	...
    	for(TextPosition otherElement : list) {
    		if(otherElement.equals(element))
    			continue;
    		BoundingBox bbOtherElement = this.boundingBoxByTextPosition.get(otherElement);
    		float otherElementXleft= bbOtherElement.getLowerLeftX();
    		float otherElementXRight= bbOtherElement.getUpperRightX();
    		float diff = 0;
    		//if the other element is at the left of the element
    		if(otherElementXRight < elementXLeft) {
    			diff = elementXLeft - otherElementXRight;
    		} else if (otherElementXleft > elementXRight) {
    			//if the other element is at the right of the element
    			diff = otherElementXleft - elementXRight;
    		}
    		...
    		if((diff < Math.max(element.getWidthOfSpace(), otherElement.getWidthOfSpace())) || isFullOverlap(bbElement, bbOtherElement)) {
    			...
    			if(element.getFontSizeInPt() != otherElement.getFontSizeInPt() ) {
        			...
        			if(localSharedY > biggestSharedArea) {
        				biggestSharedArea = localSharedY;
    ...
    {
    	//IAC create characters for each textPosition
    	List<CharacterBlock> characters = new ArrayList<CharacterBlock>();
        for(TextPosition t: textPositions) {
        	 CharacterBlock cB = new CharacterBlock(this.colorByTextPosition.get(t), this.boundingBoxByTextPosition.get(t), t);
        	 characters.add(cB);
        }
       
        //IAC create a word and add it to the current line
        if(text.length() > 0 && textPositions.size() > 0) {
        	Text word = new Text(text, characters);
        	this.currentLine.addWord(word);
        	//System.out.println("Word: " + text + " S: " + word.getStartPositionX() + " E: " + word.getEndPositionX() + " FS: " + word.getFontSize());
        }
    ...
    protected void processTextPosition(TextPosition text)
    {
    	...
    	this.colorByTextPosition.put(text, nonStrokingColor);
    ...
    protected void writeParagraphStart() throws IOException
    {
        ...
        this.currentResourceUnit.addParagraph(this.currentParagraph);
    ...
    {
//    	System.out.println("new line in normalize");{
//    		for(LineItem li : line) {
//    			if(li != null && li.getTextPosition() != null)
//    				System.out.println("\tC: " + li.getTextPosition().getUnicode());
//    		}
//    	}
        ...
        LineItem beforeItem = null;
        if(line.size() > 0)
        	beforeItem = line.get(0);
        for (LineItem item : line)
        {
            lineBuilder = normalizeAdd(normalized, lineBuilder, wordPositions, item, beforeItem);
            beforeItem = item;
        }
            ...
            StringBuilder lineBuilder, List<TextPosition> wordPositions, LineItem item, LineItem lastItem)
    {
        if (item.isWordSeparator())
        {
            ...
            wordPositions.clear();
        } else if (item.getTextPosition().getUnicode().equals(" ") || item.getTextPosition().getUnicode().equals("\u0020")) {
        	BoundingBox bB = this.boundingBoxByTextPosition.get(item.getTextPosition());
        	BoundingBox bBLast = this.boundingBoxByTextPosition.get(lastItem.getTextPosition());
        	if((bB != null && bBLast != null && bB.getUpperRightX() >= bBLast.getUpperRightX()) || bBLast == null) {
        		normalized.add(
                        createWord(lineBuilder.toString(), new ArrayList<TextPosition>(wordPositions)));
                lineBuilder = new StringBuilder();
                wordPositions.clear();
			}
        	// else: do nothing, that character is wrong and should be omitted. For example, if there are space characters between normal characters in a word
        }
        else
        {
